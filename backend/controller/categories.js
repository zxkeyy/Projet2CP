const category = require("../model/categorieSchema");
...
const getCategoryProducts = async (req, res) => {
  try {
    ...
    if (!isCategory) {
      return res
        .status(404)
        .json({ msg: `category  ${category_name} does not exist` });
    }
...
const deleteCategory = async (req, res) => {
  try {
    ...
    if (!deletedCategory) {
      return res
        .status(404)
        .json({ msg: `category with ${category_name} does not exist` });
    }
...
const editCategory = async (req, res) => {
  try {
    ...
    const editedCategory = await product.findOneAndUpdate(
      { _id: category_name },
      req.body,
      {
        runValidators: true,
    ...
    if (!editedCategory) {
      return res
        .status(404)
        .json({ msg: `category with ${product_ID} does not exist` });
    }
