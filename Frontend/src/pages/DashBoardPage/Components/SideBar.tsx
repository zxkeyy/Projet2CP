...
const SideBar = () => {
  ...
  return (
    ...
    >
      ...
      <Link to="/dashboard">
        <Flex width={"150px"} gap={"5%"} color={location.pathname == "/dashboard" ? "teal" : "black"}>
          <GoStack size={"25px"} />
          <Text fontSize={"25px"}>DashBoard</Text>
        </Flex>
      </Link>
      <Link to="/dashboard/products">
        <Flex width={"150px"} gap={"5%"} color={location.pathname == "/dashboard/products" ? "teal" : "black"}>
          <BsGrid size={"25px"} />
          <Text fontSize={"25px"}>Products</Text>
        </Flex>
      </Link>
      <Link to="/dashboard">
        <Flex width={"150px"} gap={"5%"}>
          <BsPeople size={"25px"} />
